...
(function () {
	...
	// Check if the local music server is available.
	$.getJSON(localAddress, function (data) {
		// Set the local server as available.
		localAvailable = true;
	});
	...
	function onBeforeRequest(details) {
		...
		if (qs.dl) {
			...
			var dl = encodeURIComponent(qs.dl);
			// Initialize the address.
	...
	function onHeadersReceivedListener(details) {
		...
		if (details.requestId in requests) {
			// Initialize the name.
			var name = encodeURIComponent(requests[details.requestId] + '.mp4');
